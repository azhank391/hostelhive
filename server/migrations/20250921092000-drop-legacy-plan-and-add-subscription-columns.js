...
module.exports = {
  async up(queryInterface, Sequelize) {
    ...
    const tableName = await queryInterface.describeTable('hostels').catch(() => null)
      ? 'hostels'
      : 'Hostels';
    ...
    await ensureColumn('stripe_customer_id', { type: Sequelize.STRING, allowNull: true });
    await ensureColumn('stripe_subscription_id', { type: Sequelize.STRING, allowNull: true });
    if (!desc['subscription_status']) {
      await queryInterface.addColumn(tableName, 'subscription_status', {
        type: Sequelize.ENUM('active', 'canceled', 'past_due', 'unpaid', 'trialing'),
        allowNull: true,
        defaultValue: 'trialing',
      });
    }
    await ensureColumn('current_period_start', { type: Sequelize.DATE, allowNull: true });
    await ensureColumn('current_period_end', { type: Sequelize.DATE, allowNull: true });
    await ensureColumn('trial_end', { type: Sequelize.DATE, allowNull: true });
    await ensureColumn('plan_id', { type: Sequelize.STRING(50), allowNull: true, defaultValue: 'basic' });
    ...
    if (desc['plan']) {
      ...
      if (queryInterface.sequelize.getDialect() === 'postgres') {
        await queryInterface.sequelize.query('DROP TYPE IF EXISTS "enum_' + tableName + '_plan";');
      }
    }
    ...
    // 3) Helpful indexes
    try { await queryInterface.addIndex(tableName, ['stripe_customer_id']); } catch {}
    try { await queryInterface.addIndex(tableName, ['stripe_subscription_id']); } catch {}
    try { await queryInterface.addIndex(tableName, ['subscription_status']); } catch {}
  },
  ...
  async down(queryInterface, Sequelize) {
    ...
    const dropIfExists = async (name) => {
      try { await queryInterface.removeColumn(tableName, name); } catch {}
    };
    ...
    if (queryInterface.sequelize.getDialect() === 'postgres') {
      await queryInterface.sequelize.query('DROP TYPE IF EXISTS "enum_' + tableName + '_subscription_status";');
    }
  }
};
