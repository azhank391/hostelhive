...
module.exports = {
  ...
  'SELECT id, name FROM `Permissions` WHERE name IN (:names)',
      ...
      return rows.reduce((acc, r) => { acc[r.name] = r.id; return acc; }, {});
    };
    ...
    const NEW_PERMISSIONS = [
      ...
      { name: 'export_room_data', display_name: 'Export Room Data', description: 'Export room information', category: 'rooms' },
      { name: 'export_staff_data', display_name: 'Export Staff Data', description: 'Export staff information', category: 'staff' },
      { name: 'export_complaint_data', display_name: 'Export Complaint Data', description: 'Export complaint information', category: 'complaints' }
    ];
    ...
    const LEGACY_PERMISSIONS = [
      'room_allocate', 'room_deallocate', 'visitor_checkout', 'visitor_export', 'visitor_stats_read',
      'student_export', 'hostel_stats_read', 'complaint_resolve', 'data_export', 'report_read',
      'analytics_read', 'billing_read', 'warden_create', 'warden_delete', 'warden_read', 'warden_update', 'warden_role_assign'
    ];
    ...
    if (Object.keys(legacyIds).length) {
      ...
      await queryInterface.bulkDelete('Permissions', { id: { [Op.in]: Object.values(legacyIds) } });
    }
  ...
  'SELECT id, name FROM `Roles` WHERE name IN (\'owner\',\'warden\',\'student\',\'superadmin\')',
    ...
    const roleIds = roles.reduce((acc, r) => { acc[r.name] = r.id; return acc; }, {});
  ...
  'SELECT id, name FROM `Permissions`',
    ...
    const permIdByName = allPermRows.reduce((a, r) => { a[r.name] = r.id; return a; }, {});

    const pick = (...names) => names.filter(n => permIdByName[n]).map(n => permIdByName[n]);

    // Define permission groups
    const HOSTEL_CORE = [ 'hostel_create','hostel_read','hostel_update','hostel_delete','hostel_settings_update','view_hostel_stats' ];
    const ROOMS = [ 'room_read','room_create','room_update','room_delete','room_allocation_read','room_allocation_create','room_allocation_update','room_allocation_delete' ];
    ...
    const PROFILE = [ 'manage_profile','view_profile','change_password','view_own_data' ];
    ...
    const OWNER_PERMS = [ ...HOSTEL_CORE, ...ROOMS, ...ROOMS_EXPORT, ...STUDENTS, ...STAFF, ...VISITORS, ...COMPLAINTS, ...REPORTING, ...PROFILE ];
    // Warden: same as owner minus hostel CRUD (create/update/delete/settings) but keep hostel_read + stats & exports
    const WARDEN_PERMS = OWNER_PERMS.filter(p => !['hostel_create','hostel_update','hostel_delete','hostel_settings_update'].includes(p));
    // Student: minimal
    const STUDENT_PERMS = [ 'manage_profile','view_profile','change_password','view_own_data','complaint_create','complaint_read','visitor_create','visitor_read' ];
    // Superadmin: ALL
    ...
    for (const { role, perms } of assignmentMatrix) {
      ...
      await queryInterface.bulkDelete('RolePermissions', { role_id: roleIds[role] });
      // Insert new
      const rows = perms
  ...
  async down(queryInterface, Sequelize) {
    const { Op } = Sequelize;
    const removed = [ 'export_room_data','export_staff_data','export_complaint_data' ];
  ...
  'SELECT id FROM `Permissions` WHERE name IN (:names)',
    ...
    if (ids.length) {
      await queryInterface.bulkDelete('RolePermissions', { permission_id: { [Op.in]: ids } });
