...
Object.values(API_PERMISSION_MAP).forEach(list => list.forEach(p => apiPermissions.add(p)));
...
try {
  ...
  const candidatePaths = [
    ...
    path.join(__dirname, '..', '..', '..', 'client', 'src', 'lib', 'permissionUtils.ts') // fallback
  ];
  const frontendFile = candidatePaths.find(p => fs.existsSync(p));
  if (!frontendFile) { throw new Error('permissionUtils.ts not found in candidate paths'); }
  const content = fs.readFileSync(frontendFile, 'utf8');
  const match = content.match(/export const AVAILABLE_PERMISSIONS: Permission\[] = \[(.*?)\]/s);
  if (match) {
    match[1].split(',').map(s => s.replace(/['"\s]/g,'').trim()).filter(Boolean).forEach(p => frontendPermissions.add(p));
  } else {
...
try {
  ...
  if (transMatch) {
    ...
    const keyRegex = /(\w+):/g;
    let m; const keys = [];
    while ((m = keyRegex.exec(objLiteral)) !== null) { keys.push(m[1]); }
...
async function getDbPermissions() {
  ...
  const pass = process.env.DB_PASS || process.env.MYSQL_PASSWORD || process.env.DB_PASSWORD || '';
  ...
  try {
    ...
    console.warn('⚠️ Database not reachable, skipping DB part of audit:', e.message);
    return [];
  }
}
...
async function run() {
  ...
  const codeReferenced = new Set([...backendCanonical, ...apiPermissions, ...frontendPermissions]);

  const missingInDB = setDiff(codeReferenced, dbPermissions);
  const orphanedInDB = setDiff(dbPermissions, codeReferenced);
  const unusedDefinitions = setDiff(backendCanonical, new Set([...apiPermissions, ...frontendPermissions]));
  ...
  const legacyTranslationStillReferenced = legacyTranslations.filter(lt => codeReferenced.has(lt));
  ...
  try {
    ...
    const expected = {
      owner: [ 'hostel_create','hostel_read','hostel_update','hostel_delete','hostel_settings_update','view_hostel_stats',
      ...
      warden: [ 'hostel_read','view_hostel_stats',
        ...
        // Profile
        'manage_profile','view_profile','change_password','view_own_data' ],
      student: [ 'manage_profile','view_profile','change_password','view_own_data','complaint_create','complaint_read','visitor_create','visitor_read' ],
      superadmin: Array.from(backendCanonical) // all canonical
    };

    const [roleRows] = await sequelize.query('SELECT id, name FROM Roles WHERE name IN ("owner","warden","student","superadmin")');
    const roleIdByName = Object.fromEntries(roleRows.map(r => [r.name, r.id]));
    const [rpRows] = await sequelize.query('SELECT role_id, permission_id FROM RolePermissions');
    const [permRows] = await sequelize.query('SELECT id, name FROM Permissions');
    const permNameById = Object.fromEntries(permRows.map(p => [p.id, p.name]));

    const actual = {};
    Object.entries(roleIdByName).forEach(([name, id]) => {
      const permNames = rpRows.filter(rp => rp.role_id === id).map(rp => permNameById[rp.permission_id]).filter(Boolean).sort();
    ...
    roleMatrixReport = { skipped: false, diffs, actualCounts: Object.fromEntries(Object.entries(actual).map(([r, list]) => [r, list.length])) };
    await sequelize.close();
  } catch (e) {
  ...
  const exitCode = (missingInDB.length === 0 && frontendNotInBackend.length === 0 && (!roleMatrixReport.diffs || Object.keys(roleMatrixReport.diffs).length === 0)) ? 0 : 1;
