...
exports.requireRole = (requiredRole) => {
  return (req, res, next) => {
    if (!req.user || req.user.role !== requiredRole) {
      return res
        .status(403)
        .json({ message: "Access denied: Authorized Access Only" });
    }
    next();
  };
};
...
exports.requireOwnerOrWarden = (req, res, next) => {
  ...
  console.log(
    "🔍 DEBUG: requireOwnerOrWarden - UserPermissions:",
    req.userPermissions
  );
  ...
  if (
    ["owner", "warden"].includes(req.user.role) ||
    req.userPermissions?.includes("hostel_read")
  ) {
  ...
  return res
    ...
    .json({
      message:
        "Access denied: Owner, Warden, or view_hostel permission required",
    });
};
...
exports.requireOwner = (req, res, next) => {
  if (!req.user || req.user.role !== "owner") {
    return res
      .status(403)
      .json({ message: "Access denied: Owner access required" });
  }
  next();
};
