...
import type { AuthUser, LoginCredentials, AuthContextValue } from "@/lib/types";
  ...
  const verifyToken = useCallback(
    async (token: string) => {
      try {
        ...
        if (storedUser) {
          ...
          if (userData.id === payload.id) {
            ...
            userData.requiresPasswordChange =
              payload.requiresPasswordChange || false; // Update from token
        ...
        const currentActiveHostel = localStorage.getItem(
      ...
      try {
        ...
        const userData: AuthUser = {
          ...
          role: data.role as
            ...
            | "warden"
            | "student"
            | "superadmin", // Allow any role value
        ...
        try {
          ...
          const superadminData = (await superadminApi.login({
            email,
            password,
          })) as {
            token: string;
          ...
          console.error(
            "❌ AuthContext: All login attempts failed:",
            superadminLoginError
          );
          // Normalize errors for end users
          const msg =
            superadminLoginError?.response?.data?.message ||
            userLoginError?.response?.data?.message ||
            superadminLoginError?.message ||
            userLoginError?.message ||
            "";
          if (
            /invalid\s*credentials|user\s*not\s*found|superadmin\s*not\s*found/i.test(
              msg
            )
          ) {
