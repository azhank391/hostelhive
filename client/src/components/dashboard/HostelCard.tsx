...
const StatusBadge = memo(
  ({
    ...
    const isClickable =
      onToggle && (status === "active" || status === "inactive");
      ...
      <div className="flex items-center space-x-2">
        {isClickable ? (
          <button
            ...
            title={
              status === "active" ? "Click to deactivate" : "Click to activate"
            }
          >
            {isProcessing ? (
    ...
    <div className="grid grid-cols-2 gap-3">
      {metrics.map((metric, index) => (
        <div key={index} className="text-center">
          <div
            className={`flex items-center justify-center space-x-1 ${metric.color}`}
          >
            {metric.icon}
...
export const HostelCard = memo<OptimizedHostelCardProps>(
  ({
    ...
    if (compact) {
      return (
        <div
          className={`border rounded-lg p-4 transition-all ${statusColor} ${
    ...
    return (
      ...
      >
        ...
        <div className="p-4">
            ...
            {pricing && (
              <div className="flex items-center text-sm text-gray-600 bg-gray-50 px-2 py-1 rounded-md">
                ...
                <span className="font-medium text-xs">
                  {pricing.currency || "₹"}
                  {pricing.minRent.toLocaleString()} - {pricing.currency || "₹"}
                  {pricing.maxRent.toLocaleString()}/month
                </span>
              </div>
            )}
          </div>
              ...
              <div className="flex items-center space-x-3">
                {contact.phone && (
                  <a
                    ...
                    className="flex items-center text-blue-600 hover:text-blue-800 p-1 hover:bg-blue-50 rounded-md transition-colors"
                  >
          ...
          {/* Alerts */}
          {(isNearCapacity ||
            (stats.maintenanceRequests && stats.maintenanceRequests > 5)) && (
