...
const PriorityBadge = memo(
  ({ priority, urgencyScore }: { priority: string; urgencyScore?: number }) => {
    ...
    return (
      <div
        className={`inline-flex items-center space-x-1 px-2 py-1 rounded-full text-xs font-medium border ${priorityConfig.color}`}
      >
        {priorityConfig.icon}
...
const StatusBadge = memo(
  ({ status, lastUpdated }: { status: string; lastUpdated?: string }) => {
    ...
    return (
      <div
        className={`inline-flex items-center space-x-1 px-2 py-1 rounded-full text-xs font-medium ${statusConfig.color}`}
      >
        {statusConfig.icon}
        <span>{status}</span>
        {lastUpdated && (
...
const UserAvatar = memo(
  ({
    ...
    return (
      <div
        className={`${sizeClasses} rounded-full bg-gradient-to-br from-blue-500 to-purple-600 flex items-center justify-center text-white font-medium`}
      >
...
export const ComplaintCard = memo<ComplaintCardProps>(
  ({
    ...
    const isOverdue = useMemo(() => {
      ...
      const now = new Date();
      return (
        now > resolutionDate && status !== "Resolved" && status !== "Closed"
      );
      ...
      actions.push(
        ...customActions.map((action) => ({
          ...
          condition: action.condition
            ? action.condition({ id, status, priority, reportedBy, hostel })
            : true,
        }))
      );
        ...
        <div className="p-4 sm:p-5">
          ...
          <div className="space-y-2 mb-4">
            <div className="flex items-center text-sm text-gray-600">
              ...
              {hostel.block && (
                <span className="ml-1 text-gray-500">({hostel.block})</span>
              )}
              {room && (
                <>
            ...
            <div className="flex items-center text-sm text-gray-600">
              ...
              <span>
                Reported {timeAgo} ({formattedDate})
              </span>
            </div>
            ...
            {estimatedResolution && (
              <div className="flex items-center text-sm text-gray-600">
                ...
                <span>
                  Expected resolution:{" "}
                  {new Date(estimatedResolution).toLocaleDateString()}
                </span>
              </div>
            )}
          </div>
