...
export const HostelManagement = React.memo(() => {
  ...
  const filteredHostels = useMemo(() => {
    ...
    if (searchTerm.trim()) {
      ...
      filtered = filtered.filter(
        (hostel) =>
          ...
          (hostel.email &&
            hostel.email.toLowerCase().includes(lowercaseQuery)) ||
          (hostel.subdomain &&
            hostel.subdomain.toLowerCase().includes(lowercaseQuery)) ||
          (hostel.location?.city &&
            hostel.location.city.toLowerCase().includes(lowercaseQuery))
      );
    }
    ...
    if (filterCriteria.plan_id !== "all") {
      filtered = filtered.filter(
        (hostel) => hostel.plan_id && hostel.plan_id === filterCriteria.plan_id
      );
    }
  ...
  const summaryStats = useMemo(() => {
    const stats = {
      ...
      activeHostels: hostels.filter((h) =>
        h.isActive !== undefined ? h.isActive : false
      ).length,
      paidHostels: hostels.filter((h) =>
        h.isPaid !== undefined ? h.isPaid : false
      ).length,
      totalRevenue: 0,
      totalStudents: 0,
      totalRooms: 0,
    ...
    stats.avgOccupancy =
      stats.totalRooms > 0
        ? Math.round(
            Object.values(hostelStats).reduce(
              (acc, stat) => acc + (stat.occupancyRate || 0),
              0
            ) / Object.keys(hostelStats).length || 0
          )
        : 0;
  ...
  const filterOptions = useMemo(() => {
    const plans = Array.from(
      new Set(hostels.map((h) => h.plan_id).filter(Boolean))
    );
    const countries = Array.from(
      new Set(hostels.map((h) => h.location?.country).filter(Boolean))
    );
      ...
      const statsPromises = hostels.map(async (hostel) => {
        try {
          ...
          const transformedStats: HostelStats = {
            ...
            occupancyRate:
              realStats.stats.totalRooms > 0
                ? Math.round(
                    (realStats.stats.occupiedRooms /
                      realStats.stats.totalRooms) *
                      100
                  )
                : 0,
          };
          ...
          console.error(
            `Failed to fetch stats for hostel ${hostel.id}:`,
            error
          );
  ...
  const fetchHostels = useCallback(async () => {
    try {
      ...
      const response = await apiClient.get<{ hostels: Hostel[] }>(
        "/auth/hostels/all"
      );
  ...
  const handleStatusToggle = useCallback(
    async (hostelId: string, newStatus: boolean) => {
      try {
        ...
        setHostels((prevHostels) =>
          prevHostels.map((hostel) =>
            hostel.id === hostelId ? { ...hostel, isActive: newStatus } : hostel
          )
        );
      ...
      <div className="min-h-screen bg-gradient-to-br from-gray-50 to-blue-50 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-600 mt-2">
            Please wait while we fetch your hostel information
          </p>
        </div>
      </div>
    );
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-lg text-gray-600 max-w-xl mx-auto">
            Manage all your hostels, monitor performance, and track key metrics
            ...
            <div className="flex items-center">
              ...
              <div className="ml-4">
                ...
                <p className="text-2xl font-bold text-gray-900">
                  {summaryStats.totalHostels}
                </p>
                <p className="text-xs text-green-600 font-medium">
                  {summaryStats.activeHostels} active
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white p-6 rounded-xl shadow-md border-0 hover:shadow-lg transition-all duration-300 transform hover:-translate-y-1">
            <div className="flex items-center">
              ...
              <div className="ml-4">
                ...
                <p className="text-2xl font-bold text-gray-900">
                  {summaryStats.avgOccupancy}%
                </p>
                <p className="text-xs text-gray-500">
                  {summaryStats.totalRooms} total rooms
                </p>
              </div>
            </div>
          </div>
              ...
              >
                ...
                {Object.values(filterCriteria).some((v) => v !== "all") && (
                  <span className="ml-2 bg-blue-600 text-white text-xs rounded-full w-5 h-5 flex items-center justify-center">
                    {
                      Object.values(filterCriteria).filter((v) => v !== "all")
                        .length
                    }
                  </span>
                )}
                ...
                {filteredHostels.map((hostel) => {
                  ...
                  return (
                    <HostelCard
                      ...
                      onStatusToggle={
                        canUpdateHostels ? handleStatusToggle : undefined
                      }
