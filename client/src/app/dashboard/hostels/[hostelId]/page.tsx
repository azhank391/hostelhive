    ...
    const loadDashboardData = async () => {
      try {
        ...
        if (complaintsData.data?.[0]) {
          console.log(
            "🔍 Frontend: First complaint room data:",
            complaintsData.data[0].user?.allocations?.[0]?.room?.roomNumber
          );
        }

        setComplaints(
          Array.isArray(complaintsData.data) ? complaintsData.data : []
        );
      } catch (err) {
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {complaints.slice(0, 6).map((complaint: any) => (
              <ComplaintCard
                ...
                priority={
                  ((complaint.priority || "medium").charAt(0).toUpperCase() +
                    (complaint.priority || "medium").slice(1)) as
                    | "Low"
                    | "Medium"
                    | "High"
                    | "Critical"
                ...
                room={
                  complaint.user?.allocations?.[0]?.room?.roomNumber || "N/A"
                }
                ...
                onResolve={async (complaintId: string, resolution: string) => {
                  try {
                    ...
                    const updatedComplaints =
                      await updatedComplaintsResponse.json();
                    setComplaints(
                      Array.isArray(updatedComplaints.data)
                        ? updatedComplaints.data
                        : []
                    );
                  } catch (error) {
