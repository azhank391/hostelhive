...
function BillingPageClient() {
  ...
  const preselectedPlanId = useMemo(() => {
    return selectedPlanId || (activePlan === 'basic' ? 'basic' : activePlan === 'pro' ? 'pro' : undefined);
  ...
  return (
    <div className="p-6 md:p-8 space-y-8">
      ...
      {(!subscriptionStatus?.plan_id || subscriptionStatus?.plan_id === 'free') && (
        <div className="rounded-md bg-blue-50 border border-blue-200 p-4 text-sm text-blue-900">
          ...
          <p className="mt-1">Limits: 10 rooms, 20 students, 1 warden, no visitors/complaints/staff, 1 max hostel.</p>
          <p className="mt-1">Upgrade to Basic Pro or Enterprise to unlock more capacity and features.</p>
        </div>
      )}
        ...
        <div className="rounded-md bg-amber-50 border border-amber-200 p-4 text-sm text-amber-900">
          ...
          <p className="mt-1">
            {subscriptionStatus?.trial_end
              ? `Your trial ends on ${new Date(subscriptionStatus.trial_end).toLocaleString()}.`
      ...
      {/* Canceled banner */}
      {subscriptionStatus?.subscription_status === 'canceled' && subscriptionStatus?.current_period_end && (
        <div className="rounded-md bg-amber-50 border border-amber-200 p-4 text-sm text-amber-900">
          <p className="font-medium">Subscription will end on {new Date(subscriptionStatus.current_period_end).toLocaleString()}</p>
          <p className="mt-1">You have access until the end of the current billing period. You can resume before it ends.</p>
          <div className="mt-2">
            <Button variant="outline" onClick={handleResume}>Resume subscription</Button>
          </div>
        </div>
      )}
      ...
      <Card>
        ...
        <CardContent>
          ...
          {subscriptionStatus?.current_period_start && (
            <p>Current period start: {new Date(subscriptionStatus.current_period_start).toLocaleDateString()}</p>
          )}
          {subscriptionStatus?.current_period_end && (
            <p>Current period end: {new Date(subscriptionStatus.current_period_end).toLocaleDateString()}</p>
          )}
          {subscriptionStatus?.trial_end && (
          ...
          {subscriptionStatus?.subscription_status === 'active' && (
            <p className="text-sm text-gray-500 mt-2">You have an active subscription.</p>
          )}
        </CardContent>
      </Card>

      {/* Trial CTA (collect card, no charge until after 14 days) */}
      {!['active', 'trialing', 'canceled'].includes(subscriptionStatus?.subscription_status || '') && (
      ...
      <div className="grid md:grid-cols-2 gap-6">
        {PRICING_PLANS.map((plan) => {
          const isCurrent = subscriptionStatus?.subscription_status === 'active' && subscriptionStatus?.plan_id === plan.id;
          return (
          <Card key={plan.id} className={preselectedPlanId === plan.id ? 'ring-2 ring-blue-500' : ''}>
            <CardHeader>
              <h3>{plan.name}</h3>
