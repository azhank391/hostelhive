...
export default function VisitorsPage() {
  ...
  const applyLocalFilters = useCallback(
    (visitorsData: VisitorLog[], search: string, status: string) => {
  ...
  const refreshVisitors = useCallback(async () => {
    ...
    try {
      ...
      setError(
        err instanceof Error ? err.message : "Failed to refresh visitors"
      );
    } finally {
      setLoading(false);
    }
  ...
  useEffect(() => {
    async function fetchStudents() {
      ...
      if (!hasPermission("student_read")) {
        console.log(
          "🔍 Visitors: Skipping students fetch - no view_students permission"
        );
      ...
      try {
        ...
        const studentsWithRooms = (studentsData as Student[]).filter(
          (student) => {
            ...
            if (student.allocations && Array.isArray(student.allocations)) {
              return student.allocations.some(
                (allocation) => !!allocation.room
              );
            }
  ...
  const handleCreateVisitor = useCallback(async () => {
    ...
    if (!selectedStudent || !selectedStudent.roomNumber) {
      notification.error(
        "Selected student must have an active room allocation to host visitors"
      );
      return;
    }
    ...
    try {
      ...
      notification.error(
        err instanceof Error ? err.message : "Failed to create visitor"
      );
    } finally {
  ...
  const handleCheckOutVisitor = useCallback(
    async (visitor: VisitorLog) => {
      if (!visitor || !hasHostel || !hostelId) return;

      // Add confirmation dialog
      if (
        !confirm(`Are you sure you want to check out ${visitor.visitorName}?`)
      ) {
        return;
      }
  ...
  const handleDeleteVisitor = useCallback(
    async (visitor?: VisitorLog) => {
      ...
      if (
        !confirm(
          `Are you sure you want to delete visitor ${target.visitorName}? This action cannot be undone.`
        )
      ) {
        return;
      }
  ...
  const handleViewVisitor = useCallback((visitor: VisitorLog) => {
    ...
    setFormData({
      ...
      checkIn: visitor.checkIn
        ? new Date(visitor.checkIn).toISOString().slice(0, 16)
    ...
    async (format: "csv" | "json" = "csv") => {
      ...
      try {
        await downloadExport({
          url: `/api/hostels/${hostelId}/visitors/export`,
          format,
          filename: `visitors-${hostelId}`,
        });
        notification.success(
          `Visitor data exported as ${format.toUpperCase()}!`
        );
      } catch (err) {
      ...
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-sm text-gray-500">
            Contact your administrator to get access to visitor management
            features.
          </p>
        </div>
      </div>
            ...
            <div className="flex items-center">
              ...
              <div className="ml-4">
                ...
                <p className="text-2xl font-semibold text-gray-900">
                  {
                    allVisitors.filter(
                      (v: VisitorLog) => getVisitorStatus(v) === "checked_in"
                    ).length
                  }
                </p>
              </div>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="text-sm text-gray-900">
                        {(() => {
                          ...
                          if (
                            visitor.student?.allocations &&
                            visitor.student.allocations.length > 0
                          ) {
                            const activeAllocation =
                              visitor.student.allocations.find(
                                (a) => a.status === "active"
                              );
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      ...
                      >
                        {getVisitorStatus(visitor) === "checked_in"
                          ...
                          : getVisitorStatus(visitor) === "checked_out"
                          ? "Checked Out"
                          : getVisitorStatus(visitor) === "pending"
                          ? "Pending"
                          : "Unknown"}
                      </span>
                    </td>
            ...
            <div className="grid gap-4">
              <div>
                ...
                >
                  <option value="">
                    {students.length === 0
                      ? "No students available - insufficient permissions"
                      : "Select a Host Student"}
                  </option>
      ...
      <PermissionGate permission="visitor_read">
        <Modal
          ...
          title={`Visitor Details - ${
            selectedVisitor?.visitorName || "Unknown"
          }`}
        >
                ...
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    ...
                    <p className="text-base font-semibold text-gray-900">
                      {selectedVisitor?.visitorName || "N/A"}
                    </p>
                  </div>
                </div>
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    ...
                    <p className="text-base font-semibold text-gray-900">
                      {selectedVisitor?.checkIn
                        ? new Date(selectedVisitor.checkIn).toLocaleString(
                            "en-US",
                            {
                              year: "numeric",
                              month: "long",
                  ...
                  <div>
                    ...
                    <p className="text-base font-semibold text-gray-900">
                      {selectedVisitor?.checkOut
                        ? new Date(selectedVisitor.checkOut).toLocaleString(
                            "en-US",
                            {
                              year: "numeric",
                              month: "long",
                  ...
                  <div className="w-5 h-5 rounded-full bg-gray-200 flex items-center justify-center">
                    <div
                      className={`w-3 h-3 rounded-full ${
                        selectedVisitor &&
                        getVisitorStatus(selectedVisitor) === "checked_in"
                          ? "bg-green-500"
                          : selectedVisitor &&
                            getVisitorStatus(selectedVisitor) === "checked_out"
                          ? "bg-gray-500"
                          : "bg-yellow-500"
                      }`}
                    ></div>
                    ...
                    <p className="text-base font-semibold text-gray-900">
                      {selectedVisitor
                        ? getVisitorStatus(selectedVisitor) === "checked_in"
                          ...
                          : getVisitorStatus(selectedVisitor) === "checked_out"
                          ? "Checked Out"
                          : getVisitorStatus(selectedVisitor) === "pending"
                          ? "Pending"
                          : "Unknown"
                        : "N/A"}
                    </p>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    ...
                    <p className="text-base font-semibold text-gray-900">
                      {selectedVisitor?.student?.name || "N/A"}
                    </p>
                  </div>
                </div>
                <div className="flex items-center space-x-3">
                ...
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    ...
                    <p className="text-base font-semibold text-gray-900">
                      {(() => {
                        const student = students.find(
                          (s) => s.id === selectedVisitor?.studentId
                        );
              ...
              <PermissionGate permission="visitor_update">
                {selectedVisitor &&
                  getVisitorStatus(selectedVisitor) === "checked_in" && (
                    <Button
                      variant="primary"
                      onClick={() => handleCheckOutVisitor(selectedVisitor)}
